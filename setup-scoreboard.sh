#!/usr/bin/env bash
# setup-scoreboard.sh - All-in-one setup for CDCWebSim SCOREBOARD on Kali
# - Repairs apt trust/time/sources, installs deps
# - Clones/updates public repo (HTTPS only; ZIP fallback)
# - Creates venv, installs requirements (+ ensures gunicorn)
# - Creates Postgres role+DB (enabled), writes .env
# - systemd service: ctf-scoreboard.service (gunicorn -> 127.0.0.1:8001)
# - nginx reverse proxy: listen 8081 -> 127.0.0.1:8001
...
ROLE="scoreboard"
SERVICE="ctf-scoreboard.service"
...
APP_PORT="${APP_PORT:-8001}"     # Gunicorn bind
NGINX_PORT="${NGINX_PORT:-8081}" # External port
NGINX_SITE_NAME="maccdc-${ROLE}"

# DB enabled for scoreboard
...
# ----- commands -----
install_all(){
  require_root; ensure_user; ensure_dirs; ensure_packages; clone_or_update; create_venv; setup_postgres; write_env; write_systemd; write_nginx; summary
  log "install-all complete. Next: sudo ./setup-${ROLE}.sh start-everything"
}
